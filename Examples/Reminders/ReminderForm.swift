...
struct ReminderFormView: View {
  @SharedReader(
    .supabase(
      "reminders_lists",
      filter: { @Sendable in $0.order("name") }
    )
  )
  ...
  private func saveButtonTapped() {
    Task {
      await withErrorReporting {
        try await supabase.from(Reminder.databaseTableName).upsert(reminder).execute()
        try await supabase.from(ReminderTag.databaseTableName).delete().eq(
          "reminder_id", value: reminder.id!
        ).execute()
        for tag in selectedTags {
          try await supabase.from(ReminderTag.databaseTableName).insert(
            ReminderTag(reminderID: reminder.id!, tagID: tag.id!)
          ).execute()
        }
      }
      dismiss()
...
extension Reminder {
  fileprivate var isDateSet: Bool {
    get { date != nil }
    set { date = newValue ? Calendar.current.naiveDate(from: Date()) : nil }
  }
}
extension Optional {
...
struct TagsPopover: View {
  @SharedReader(
    .supabase(
      "tags",
      filter: { @Sendable in $0.order("name", ascending: true) }
    )
  )
